...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.domain_classifier.add_module('fc1',nn.Linear(config.hidden_size,2))
        # self.domain_classifier.add_module('relu1',nn.ReLU())
        # self.domain_classifier.add_module('fc2',nn.Linear(128,2))
        #self.domain_classifier.add_module('softmax',nn.LogSoftmax(dim=1))
        nn.init.xavier_uniform_(self.domain_classifier.fc1.weight)
        nn.init.constant_(self.domain_classifier.fc1.bias,0.3)
        #nn.init.xavier_uniform_(self.domain_classifier.fc2.weight)
        #nn.init.constant_(self.domain_classifier.fc2.bias,0.3)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        if domain_labels is not None:
            ...
            domain_loss = loss_fct(domain_logits.view(-1,2),domain_labels.view(-1))
        loss = None
        if labels is not None:
            if self.config.problem_type is None:
                if self.num_labels == 1:
